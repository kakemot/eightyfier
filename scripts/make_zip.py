...
def  main():
  ...
  for line in fileinput.input(projectpath + "\config.h",inplace=0):
    ...
    if "#define BUNDLE_NAME " in line:
      BUNDLE_NAME = "#define BUNDLE_NAME ".lstrip(line)

  FULLVERSIONSTR = MAJORSTR + "." + MINORSTR + "." + BUGFIXSTR

  ZIPNAME = "Eightyfier-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Eightyfier-v" + FULLVERSIONSTR + "-win-demo.zip"
